...
def register(request):
    if request.method=="POST":
        ...
        if form.is_valid():
            ...
            # messages.success(request, 'Thank you for registration with us we have to sent you on verification email to your email address')
...
def forgotPassword(request):
    if request.method=="POST":
        ...
        if Account.objects.filter(email=email).exists():
            ...
            message = render_to_string('account/reset_passwrod_email.html',{
                'user':user,
                'domain':current_site,
                'uid':urlsafe_base64_encode(force_bytes(user.id)),
                'token':default_token_generator.make_token(user)
